import { ExampleRequest } from '../src/requests/ExampleRequest'

// mock objects to be used in the tests
// To avoid having always the same mock objects copied and pasted around the whole unit tests suite
// example:

// export const mockAddress = '0x5d2FD0EFb594179D3B772640f8dA975871e460d2'
// export const mockContractAddress = '0x32A9daeD647a8CC42FfBAd1498BC32074b0ae0A8'
// export const mockAmount = 100
// export const mockTotalSupply = 100
// ... etc ...

export const validExampleRequest: ExampleRequest = {
  value: 'things',
}

export const validExampleParam = 'someparam'
